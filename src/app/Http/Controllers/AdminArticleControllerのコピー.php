...
class AdminArticlesConstruct extends DataBox{
  public function __construct()
  {
    $this->models = new ArticleModels();
    $this->adminUserDataControllers = new AdminUsersController();

  }
}

class AdminArticlesController extends AdminArticlesConstruct{
  ...
  public function getArticle(){
    ...
    if ($this->adminUserDataControllers->adminCheck()["0"]==="1") {
      require_once(dirname(__FILE__) . $_ENV["ADMIN_DIRECTRY"] . "article.php");
    }
  }


  public function download(){
    $file_path = "/Applications/XAMPP/xamppfiles/htdocs/bulletin_board/article.csv";
    $export_csv_title = ["ID","user_id", "title","content","created_at","updated_at","deleted_at"];
  ...
  public function download_file($path_file){
    ...
    require_once(dirname(__FILE__) . $_ENV["ADMIN_DIRECTRY"] . "article_download.php");
    /* ファイルを読んで出力 */
  ...
  {     
    if (isset($_FILES['upfile']['error']) && is_int($_FILES['upfile']['error'])) {
      try {
        ...
        switch ($_FILES['upfile']['error']) {
          ...
          // サイズ確認
            throw new RuntimeException('ファイルが大き過ぎます');
            default:
        ...
        unset($buffer);
        $executed= $this->data = $this->models->insert($tmp_name);
            /* 結果メッセージをセット */
        if (isset($executed)) {
    ...
    if ($this->adminUserDataControllers->adminCheck()["0"]==="1") {
      require_once(dirname(__FILE__) . $_ENV["ADMIN_DIRECTRY"] . "upload_article.php");
    }
  }
}